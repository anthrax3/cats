...
namespace
{
	...
	float 
	log10scale_inv(float val) 
	{
		assert(val >= 0 && val <= 1);
		return ( std::pow(10, val) - 1 ) / 9;
	}
	
	double cent2freq(double cents) { return std::pow(2, cents / 1200); }
	double freq2cent(double freq) { return std::log(freq) / std::log(2) * 1200; }
	double cent2oct(double cents) { return cents / 1200; }
	double oct2cent(double oct) { return oct * 1200; }
	void shift90deg(Complex& x) { x = std::conj(Complex(x.imag(), x.real())); }
	
	real_vec 
	resample(const real_vec& in, size_t len) {
		assert(len > 0);
		...
		SRC_DATA parms = { const_cast<float*>( &in[ 0 ] ), &out[ 0 ], in.size(), out.size(), 0, 0, 0, ratio };
	...
	get_envelope(complex_vec& band) {
		...
		for ( size_t i = 0; i < envelope.size(); ++i )
			envelope[ i ] = std::sqrt(envelope[ i ] * envelope[ i ] + shifted_signal[ i ] * shifted_signal[ i ]);
	...
	double 
	window_coef(double x, window_type window)
	{
		assert(x >= 0 && x <= 1);

		if ( window == WINDOW_TYPE_RECTANGULAR )
			return 1.0;
		...
		switch ( window ) {
			...
			case WINDOW_TYPE_BLACKMAN:
				x = blackman_window(x);
				break;
			case WINDOW_TYPE_TRIANGULAR:
				x = triangular_window(x);
				break;
			default:
				break;
		}
	...
	float 
	calc_intensity(float val, axisScale_t intensity_axis) 
	{
		assert(val >= 0 && val <= 1);
	...
	float 
	calc_intensity_inv(float val, axisScale_t intensity_axis) 
	{
	...
	void 
	normalize_image(std::vector<real_vec>& data) 
	{
		float max = 0.0f;

		for ( std::vector<real_vec>::iterator it = data.begin(); it != data.end(); ++it )
	...
	// to <-1,1>
	void 
	normalize_signal(real_vec& vector) 
	{
		float max = 0;
	...
	float 
	brightness_correction(float intensity, bcorrection_t correction) 
	{
	...
	scale_hz(double f)
	{
		...
		if ( 1. <= ( f / 1000000000 ) ) {
			r = QString::number(f / 1000000000) + "Ghz";
		} else if ( 1. <= ( f / 1000000 ) ) {
			r = QString::number(f / 1000000) + "Mhz";
		} else if ( 1. <= ( f / 1000 ) ) {
			r = QString::number(f / 1000) + "Khz";
		} else {
			r = QString::number(f) + "Hz";
		}
...
// -- bandwidth will be 107.1hz and basefreq will hit the std::max and start at ~3hz
spectrogram_t::spectrogram_t(QObject* p) : QObject(p), m_bandwidth(7), m_basefreq(3.75), m_maxfreq(22050), m_overlap(0.8),
						m_ppsec(100), m_window(WINDOW_TYPE_HANN), m_iaxis(SCALE_LINEAR), m_correction(BRIGHT_SQRT),
						m_faxis(SCALE_LINEAR)
{
}

/*spectrogram_t::spectrogram_t(QObject* p) : QObject(p), m_bandwidth(25), m_basefreq(55), m_maxfreq(22050), m_overlap(0.8),
						m_ppsec(100), m_window(WINDOW_HANN), m_iaxis(SCALE_LOGARITHMIC), m_correction(BRIGHT_NONE),
						m_faxis(SCALE_LOGARITHMIC)
{
}*/

QImage 
spectrogram_t::to_image(real_vec& signal, int samplerate)
{
	const double				mfreq = ( samplerate/2 < m_maxfreq ? samplerate/2 : m_maxfreq );
	const complex_vec			spectrum = padded_FFT(signal);
	const size_t				width = ( spectrum.size() - 1 ) * 2 * m_ppsec / samplerate;
	const double				filterscale = ( (double)spectrum.size() * 2 ) / samplerate;
	std::auto_ptr<Filterbank>	filterbank = Filterbank::get_filterbank(m_faxis, filterscale, m_basefreq, m_bandwidth, m_overlap);
	const int					bands = filterbank->num_bands_est(mfreq);
	const int					top_index = mfreq*filterscale;
	...
	for ( size_t bandidx = 0;; ++bandidx ) {
		...
		//emit updateStatus("[Band number: " + QString::number(bandidx) + "]:\t" + scale_hz(range.first) + " through " + scale_hz(range.second));

		if ( range.first > top_index )
			break;

		std::copy(spectrum.begin() + range.first, spectrum.begin() + std::min(range.second, top_index), filterband.begin());
		
		if ( range.second > top_index ) 
			std::fill(filterband.begin() + (top_index - range.first), filterband.end(), Complex(0, 0));
...
QImage 
spectrogram_t::make_image(const std::vector<real_vec>& data)
{
	const size_t height = data.size();
	const size_t width = data[ 0 ].size();
	QImage out(width, height, QImage::Format_RGB32); //palette.make_canvas(width, height);
	...
	for ( size_t y = 0; y < height; ++y ) {
		...
		for ( size_t x = 0; x < width; ++x ) {
			float intensity = calc_intensity(data[ y ][ x ], m_iaxis);
			intensity = brightness_correction(intensity, m_correction);
			rgb_t col = m_cmap.getColor(intensity);
			
			out.setPixel(x, ( height - 1 - y ), QColor::fromRgbF(col.r, col.g, col.b).rgba());
		}
	}
...
void 
spectrogram_t::apply_window(complex_vec& chunk, int lowidx, double filterscale)
{
	const int highidx = lowidx + chunk.size();
	
	if ( SCALE_LINEAR == m_faxis )
		for ( size_t i = 0; i < chunk.size(); ++i )
			chunk[ i ] *= window_coef((double)i / ( chunk.size() - 1 ), m_window);
	else {
		...
		for ( size_t i = 0; i < chunk.size(); ++i ) {
			...
			chunk[ i ] *= window_coef(winidx, m_window);
		}
	}
}

/*real_vec 
Spectrogram::synthetize(const QImage& image, int samplerate, SynthesisType type) 
{
	switch ( type ) {
		case SYNTHESIS_SINE:
...
/*real_vec 
Spectrogram::sine_synthesis(const QImage& image, int samplerate) 
{
	...
	std::auto_ptr<Filterbank> filterbank = Filterbank::get_filterbank(frequency_axis, filterscale, 
																	basefreq, bandwidth, overlap);
	...
	for ( int bandidx = 0; bandidx < image.height(); ++bandidx ) {
		...
		for ( size_t i = 0; i < filterband.size(); ++i ) {
			...
			filterband[ i ] *= x - ( ( 0.5 / ( 2.0*PI ) )*sin(2.0*PI*x) + ( 0.08 / ( 4.0*PI ) )*sin(4.0*PI*x) / 0.42 );
		}
...
/*real_vec 
Spectrogram::noise_synthesis(const QImage& image, int samplerate) 
{
	...
	std::auto_ptr<Filterbank> filterbank = Filterbank::get_filterbank(frequency_axis, filterscale, basefreq, bandwidth, overlap);
	...
	for ( int bandidx = 0; bandidx < image.height(); ++bandidx ) {
		...
		real_vec envelope = resample(envelope_from_spectrogram(image, bandidx), samples);
...
/*void 
Spectrogram::band_progress(int x, int of, int from, int to) 
{
	QString bandstatus;
...
/*real_vec 
Spectrogram::envelope_from_spectrogram(const QImage& image, int row) 
{
	real_vec envelope(image.width());

	for ( int x = 0; x < image.width(); ++x )
		envelope[ x ] = calc_intensity_inv(palette.get_intensity(image.pixel(x, image.height() - row - 1)), intensity_axis);

	return envelope;
}*/

/*void 
Spectrogram::deserialize(const QString& text)
{
...
Spectrogram::serialized() 
{
	...
	desc << "Spectrogram:" << delimiter << bandwidth << delimiter << basefreq << delimiter << maxfreq << delimiter << overlap * 100 << delimiter
		<< pixpersec << delimiter << (int)window << delimiter << (int)intensity_axis << delimiter << (int)frequency_axis << delimiter;
...
int 
Palette::get_color(float val) const
{
	assert(val >= 0 && val <= 1);

	if ( indexable() ) // returns the color index
		return ( colors_.size() - 1 )*val;
	else // returns the RGB value
		return colors_[ ( colors_.size() - 1 )*val ];
}

bool 
Palette::has_color(QRgb color) const
{
...
// ne moc efektivni
float 
Palette::get_intensity(QRgb color) const
{
...
QImage 
Palette::make_canvas(int width, int height) const
{
	if ( 0 ) { //indexable() ) {
...
bool 
Palette::indexable() const
{
	return colors_.size() <= 256;
}

QPixmap 
Palette::preview(int width, int height) const
{
...
int 
Palette::numColors() const
{
	return colors_.size();
}*/

Filterbank::Filterbank(double scale) : scale_(scale)
{
}
...
LinearFilterbank::LinearFilterbank(double scale, double base, double hzbandwidth, double overlap) : Filterbank(scale),  
						bandwidth_(hzbandwidth*scale), startidx_(std::max(scale_*base - bandwidth_ / 2, 0.0)), 
...
int 
LinearFilterbank::num_bands_est(double maxfreq) const
{
	return ( maxfreq*scale_ - startidx_ ) / step_;
}

intpair 
LinearFilterbank::get_band(int i) const
{
	intpair out;
...
int 
LinearFilterbank::get_center(int i) const
{
	return startidx_ + i*step_ + bandwidth_ / 2.0;
}

LogFilterbank::LogFilterbank(double scale, double base, double centsperband, double overlap) : Filterbank(scale), 
										centsperband_(centsperband), logstart_(freq2cent(base)), 
...
int 
LogFilterbank::num_bands_est(double maxfreq) const
{
	return ( freq2cent(maxfreq) - logstart_ ) / logstep_ + 4;
}

int 
LogFilterbank::get_center(int i) const
{
...
intpair 
LogFilterbank::get_band(int i) const
{
	const double logcenter = logstart_ + i*logstep_;
...
std::auto_ptr<Filterbank> 
Filterbank::get_filterbank(axisScale_t type, double scale, double base, double bandwidth, double overlap)
{
	Filterbank* filterbank;
